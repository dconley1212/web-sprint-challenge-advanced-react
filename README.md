...
1. What are the main differences between a class-based and a functional component?
   Class based components have a few key differences to functional based components. The first is there is different syntax used like class extending React.Component or the render method. Also, there are different lifecyle method syntax like component did mount, component did update, or component will unmount. So there is a little more readibility into when you want the code to execute. Even though functional components have similar capability with the useEffect function. Additionally, Functionally components are more efficient to create as a developer because it takes less code to write, but functional components have less structure than class components so they can be harder to read and understand at times.

2. When does a componentWillMount function be called? What about a componentDidUpdate?
   Component Will Mount is consider legacy code instead you should use componentDidMount and it is called after the render method has been called and executed. ComponentDidUpdate will be called after the componentDidMount.

3. Define stateful logic.
   Stateful logic is when code is written to create or manipulate state data in react.

4. What are the three step of creating a successful test? What is done in each phase?
   Arrange, Act, and Assert. Arrange means you render the right app to the screen. Act is when you copy the behavior of what the user would do when trying to preform a certain action on the screen like clicking a button or filling in input fields. Assert is the determining the response to the behavior in the user event.
